...
export function SagasView() {
  ...
  if (loading) {
    return <LoadingIndicator label="Loading sagas..." />;
  }
  ...
  const avgConfidence =
    sagas.length > 0
      ? sagas.reduce((sum, s) => sum + s.confidence, 0) / sagas.length
      : 0;
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.list}>
        ...
        {sagas.length === 0 && (
          <div className={styles.empty}>No sagas found</div>
        )}
      </div>
    </div>
  );
