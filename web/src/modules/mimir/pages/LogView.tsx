...
import type { MimirLogEntry } from '../api/types';
...
export function LogView() {
  ...
  const fetchLog = useCallback(async () => {
    ...
    setError(null);
    try {
      const entry = await mimirClient.getLog(lineCount);
      setLogEntry(entry);
  ...
  return (
    <div className={styles.page}>
      ...
      {!error && !logEntry && loading && <div className={styles.loading}>Loading log...</div>}
      {logEntry && (
        <div className={styles.content}>
          <LogViewer entries={logEntry.entries} filter={filter} onFilterChange={setFilter} />
        </div>
      )}
    </div>
  );
