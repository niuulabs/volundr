...
describe('DreamsPage', () => {
  ...
  it('renders dream cycle entries after load', async () => {
    wrap(<DreamsPage />);
    await waitFor(() =>
      expect(screen.getAllByTestId('dream-cycle').length).toBeGreaterThan(0),
    );
  });
  ...
  it('shows pages updated count', async () => {
    wrap(<DreamsPage />);
    await waitFor(() =>
      expect(screen.getAllByTestId('dream-pages').length).toBeGreaterThan(0),
    );
  ...
  it('shows entities created count', async () => {
    wrap(<DreamsPage />);
    await waitFor(() =>
      expect(screen.getAllByTestId('dream-entities').length).toBeGreaterThan(0),
    );
  ...
  it('shows lint fixes count', async () => {
    wrap(<DreamsPage />);
    await waitFor(() =>
      expect(screen.getAllByTestId('dream-fixes').length).toBeGreaterThan(0),
    );
  ...
  it('shows error state when service throws', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText('dreams service down')).toBeInTheDocument(),
    );
  });
  ...
  it('shows empty message when no cycles exist', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText(/no dream cycles recorded/i)).toBeInTheDocument(),
    );
  });
