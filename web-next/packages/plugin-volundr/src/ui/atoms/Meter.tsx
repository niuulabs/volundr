...
export function Meter({
  ...
  if (used == null || limit == null || limit === 0) {
    return (
      <div className={cn('niuu-flex niuu-flex-col niuu-gap-0.5', className)} data-testid="meter">
        ...
        <div className="niuu-h-1.5 niuu-rounded-full niuu-bg-bg-elevated" role="meter" aria-valuenow={0} aria-valuemin={0} aria-valuemax={100} aria-label={label ?? 'meter'}>
  ...
  const colorClass =
    level === 'hot' ? 'niuu-bg-critical' : level === 'warm' ? 'niuu-bg-state-warn' : 'niuu-bg-brand';
  const pctNum = Math.round(pct * 100);

  return (
    <div className={cn('niuu-flex niuu-flex-col niuu-gap-0.5', className)} data-testid="meter" data-level={level}>
      {label && (
        <div className="niuu-flex niuu-justify-between niuu-text-xs">
          <span className="niuu-text-text-muted">{label}</span>
