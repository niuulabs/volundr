...
def validator(mock_repo: AsyncMock) -> PATValidator:
    return PATValidator(repo=mock_repo, signing_key=SIGNING_KEY, cache_ttl=300)
...
class TestIsValid:
    ...
    async def test_jwt_without_type_passes_through(
        self, validator: PATValidator, mock_repo: AsyncMock,
    ):
...
class TestCaching:
    ...
    async def test_cache_expires(self, mock_repo: AsyncMock):
        validator = PATValidator(repo=mock_repo, signing_key=SIGNING_KEY, cache_ttl=0.01)
        token = _make_pat_jwt()
