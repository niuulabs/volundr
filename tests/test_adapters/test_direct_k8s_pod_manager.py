...
class TestCredentialFileMounting:
    """Test credential file volume and init container generation."""

    def test_home_setup_init_container_added_with_credentials(
        self,
        pod_manager: DirectK8sPodManager,
        sample_session: Session,
    ) -> None:
        spec = make_spec(
            homeVolume={
                ...
                "credentialFiles": {
                    ...
                    "secretMountPath": "/volundr/secrets/creds",
                    "destDir": ".claude",
                },
            },
        )
        init_containers = pod_manager._build_init_containers(sample_session, spec)
        names = [c["name"] for c in init_containers]
        assert "home-setup" in names

        home_setup = next(c for c in init_containers if c["name"] == "home-setup")
        mount_names = [m["name"] for m in home_setup["volumeMounts"]]
        assert "home" in mount_names
        assert "credential-files" in mount_names

        cred_mount = next(m for m in home_setup["volumeMounts"] if m["name"] == "credential-files")
        assert cred_mount["readOnly"] is True
        assert cred_mount["mountPath"] == "/volundr/secrets/creds"

    def test_no_home_setup_without_credential_secret(
        self,
        pod_manager: DirectK8sPodManager,
        sample_session: Session,
    ) -> None:
        ...
        init_containers = pod_manager._build_init_containers(sample_session, spec)
        names = [c["name"] for c in init_containers]
        assert "home-setup" not in names

    def test_no_home_setup_when_home_disabled(
        self,
        pod_manager: DirectK8sPodManager,
        sample_session: Session,
    ) -> None:
        spec = make_spec(
            homeVolume={
                "enabled": False,
                "credentialFiles": {"secretName": "claude-credentials"},
            },
        )
        init_containers = pod_manager._build_init_containers(sample_session, spec)
        names = [c["name"] for c in init_containers]
    ...
    def test_credential_volume_added_to_deployment(
        self,
        pod_manager: DirectK8sPodManager,
        sample_session: Session,
    ) -> None:
        ...
        cred_vol = next((v for v in volumes if v["name"] == "credential-files"), None)
        assert cred_vol is not None
        assert cred_vol["secret"]["secretName"] == "claude-credentials"
        assert cred_vol["secret"]["optional"] is True

    def test_claude_config_dir_set_with_home_volume(
        self,
        pod_manager: DirectK8sPodManager,
        sample_session: Session,
    ) -> None:
        ...
        assert env_dict["CLAUDE_CONFIG_DIR"] == "/volundr/home/.claude"

    def test_claude_config_dir_default_dest_dir(
        self,
        pod_manager: DirectK8sPodManager,
        sample_session: Session,
    ) -> None:
        """When no destDir specified, defaults to .claude."""
        spec = make_spec(
            homeVolume={
                "enabled": True,
                "existingClaim": "volundr-user-home",
