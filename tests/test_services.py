...
class TestFarmWaitForReady:
    """Tests for Farm adapter wait_for_ready polling."""

    @pytest.mark.asyncio
    async def test_farm_wait_for_ready_returns_running(self):
        """Farm wait_for_ready returns RUNNING when status is running."""
        from unittest.mock import AsyncMock, patch

        from volundr.adapters.outbound.farm import FarmPodManager

        farm = FarmPodManager(base_url="http://fake", poll_interval=0.01)
        ...
        with patch.object(farm, "status", new_callable=AsyncMock) as mock_status:
            mock_status.side_effect = [
                SessionStatus.STARTING,
                SessionStatus.STARTING,
                SessionStatus.RUNNING,
            ]
            result = await farm.wait_for_ready(session, timeout=5.0)

        assert result == SessionStatus.RUNNING

    @pytest.mark.asyncio
    async def test_farm_wait_for_ready_returns_failed_on_error(self):
        """Farm wait_for_ready returns FAILED when status is failed."""
        from unittest.mock import AsyncMock, patch
        ...
        with patch.object(farm, "status", new_callable=AsyncMock) as mock_status:
            mock_status.return_value = SessionStatus.FAILED
            result = await farm.wait_for_ready(session, timeout=5.0)

        assert result == SessionStatus.FAILED

    @pytest.mark.asyncio
    async def test_farm_wait_for_ready_returns_failed_on_timeout(self):
        """Farm wait_for_ready returns FAILED when timeout expires."""
        from unittest.mock import AsyncMock, patch
        ...
        with patch.object(farm, "status", new_callable=AsyncMock) as mock_status:
            ...
            result = await farm.wait_for_ready(session, timeout=0.05)
